  ...
  return (
    <div className="min-h-screen grid lg:grid-cols-2">
      ...
      <div className="hidden lg:flex flex-col justify-center items-start p-12 xl:p-16 relative overflow-hidden"
        ...
        <div className="absolute bottom-[-100px] right-[-100px] w-96 h-96 rounded-full pointer-events-none"
        ...
        <h1 className="font-display text-4xl xl:text-5xl font-extrabold text-white leading-tight tracking-tight mb-4">
          Comida boa,<br />entregue <span className="text-primary">rápido</span>
        </h1>
        ...
        <div className="flex flex-col gap-4">
          {[
            ...
            <div key={text} className="flex items-center gap-3 text-white/80 text-sm font-semibold">
              <div className="w-9 h-9 bg-white/10 rounded-xl flex items-center justify-center text-lg flex-shrink-0">{icon}</div>
              {text}
            </div>
          ))}
        </div>
      ...
      <div className="flex items-center justify-center p-4 sm:p-8 bg-surface-2">
        <div className="bg-white rounded-3xl shadow-2xl border border-border p-8 sm:p-10 w-full max-w-100">
          ...
          <h2 className="font-display text-2xl font-extrabold text-text-primary mb-1 tracking-tight">Bem-vindo de volta!</h2>
          <p className="text-sm text-text-muted font-semibold mb-8">Entre na sua conta para continuar</p>

          <form onSubmit={handleEnviar} className="flex flex-col gap-4">
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-extrabold text-text-secondary uppercase tracking-wide">E-mail ou Telefone</label>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="relative">
                <input type={mostrarSenha ? 'text' : 'password'} placeholder="••••••••" value={senha} onChange={e => setSenha(e.target.value)} required
            ...
            <a href="#" className="text-xs text-primary font-bold self-end hover:underline">Esqueceu a senha?</a>
          ...
          <p className="text-center text-sm text-text-secondary font-semibold">
            Não tem uma conta?{' '}
            <button onClick={() => navigate('/register/user')} className="text-accent font-extrabold bg-transparent border-none cursor-pointer hover:underline">
              Cadastre-se grátis
            </button>
          </p>
          ...
          >🏪 Cadastre seu restaurante ou mercado</button>
        </div>
      </div>
    </div>
  )
