...
import { LayoutDashboard, ShoppingBag, UtensilsCrossed, BarChart3, Users, Settings, LogOut, Store } from 'lucide-react'
    ...
    <header className="bg-white border-b border-border sticky top-0 z-50 shadow-[0_2px_8px_rgba(0,0,0,0.08)]">
      ...
      <div className="hidden md:flex bg-secondary px-6 py-1.5 justify-between items-center">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[0.78rem] text-white/70 font-semibold">Painel do Gerente • <strong className="text-white/90">{nomeLoja}</strong></span>
        </div>
        <div className="flex items-center gap-4">
          <span className="text-[0.78rem] text-white/60 font-semibold">Hoje: <strong className="text-white/90">{new Date().toLocaleDateString('pt-BR', { weekday: 'short', day: '2-digit', month: 'short' })}</strong></span>
          <span className="text-[0.78rem] text-white/60 font-semibold">Status: <strong className="text-accent">Aberto</strong></span>
        </div>
      </div>

      {/* Main */}
      <div className="max-w-320 mx-auto px-6 h-14 flex items-center gap-0">
        <Link to="/" className="flex items-center gap-2 mr-6 flex-shrink-0">
          <img src={logoSrc} alt="FoodExpress" className="h-9 w-auto" />
        </Link>

        <span className="hidden lg:block text-[0.72rem] font-extrabold text-secondary bg-[rgba(46,41,78,0.08)] px-2.5 py-1 rounded-md uppercase tracking-wider mr-6 flex-shrink-0">Gerente</span>
        ...
        <div className="flex items-center gap-2 ml-auto flex-shrink-0">
          ...
          <div onClick={() => navigate('/perfil')} className="flex items-center gap-2 bg-surface-2 border border-border px-3 pr-4 py-1.5 rounded-full cursor-pointer transition-all hover:border-primary hover:bg-primary-light">
            <div className="w-7 h-7 bg-secondary rounded-full flex items-center justify-center text-white text-[0.8rem] font-bold">{nomeGerente.charAt(0).toUpperCase()}</div>
            <span className="hidden sm:block text-[0.85rem] font-bold text-text-primary">{nomeGerente.split(' ')[0]}</span>
          </div>
