...
// Rotas do funcionário — ajustadas para o padrão do App.jsx
const links = [
  { to: '/funcionario', label: 'Dashboard', Icon: LayoutDashboard },
  { to: '/funcionario/pedidos', label: 'Meus Pedidos', Icon: ShoppingBag },
  { to: '/funcionario/historico', label: 'Histórico', Icon: Clock },
  { to: '/funcionario/suporte', label: 'Suporte', Icon: Headphones },
];
  ...
  return (
    <header className="bg-white border-b border-border sticky top-0 z-50 shadow-[0_2px_8px_rgba(0,0,0,0.08)]">
      ...
      <div className="max-w-7xl mx-auto px-6 h-14 flex items-center">
        ...
        <span className="hidden lg:block text-[0.72rem] font-extrabold text-accent bg-accent/10 px-2.5 py-1 rounded-md uppercase tracking-wider mr-6 shrink-0">
          Funcionário
        </span>
        ...
        <div className="flex items-center gap-2 ml-auto">
          <div className="hidden sm:flex items-center gap-1.5 border border-accent/20 bg-accent/10 text-accent px-3 py-1.5 rounded-full text-[0.78rem] font-extrabold cursor-pointer">
            <div className="w-2 h-2 bg-text-muted rounded-full" />
            Offline
          </div>

          <div onClick={() => navigate('/perfil')}
            className="flex items-center gap-2 bg-surface-2 border border-border px-3 pr-4 py-1.5 rounded-full cursor-pointer hover:border-accent hover:bg-accent/10 transition-all"
          >
            <div className="w-7 h-7 bg-accent rounded-full flex items-center justify-center text-white text-[0.8rem] font-bold">
